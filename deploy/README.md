...
`ec2:DescribeSecurityGroups` and `ec2:DescribeVpcs` are for NET-002 (verifies the cluster's referenced security groups and VPC still exist — an 11th check added beyond the original locked 10, after real-world research surfaced `SecurityGroupNotFound`/`VpcIdNotFound` as common EKS control-plane upgrade failures).
