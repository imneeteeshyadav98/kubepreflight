...
| WH-005 | Webhook unsafe scope and timeout: `timeoutSeconds` near the 30s API ceiling, `operations: ["*"]` (includes CONNECT), self-interception (rules cover the webhook config resources themselves), or coverage of cluster-critical resources (nodes, namespaces, persistentvolumes) | Webhook rules, selectors, timeoutSeconds | Warning regardless of failurePolicy -- WH-005 flags scope/configuration risk, not a confirmed admission failure; the message text distinguishes `failurePolicy: Fail` (a more direct dependency) from `failurePolicy: Ignore` (a configuration risk worth reviewing) | `STATIC_CERTAIN` |
