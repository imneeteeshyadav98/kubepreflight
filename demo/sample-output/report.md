...
### `P4` `WH-001` ValidatingWebhookConfiguration "demo-catchall-guard": webhook "guard.demo.kubepreflight.io" is fail-closed with catch-all resource rules (apiGroups: ["*"], resources: ["*"], operations: [CREATE, UPDATE]) — all namespaces and cluster-scoped resources depend on this webhook's backend being healthy
...
2. **[P2/Blocker] PodDisruptionBudget/demo/shared-app-pdb-a** (API-001, API-001, PDB-001, PDB-002)
